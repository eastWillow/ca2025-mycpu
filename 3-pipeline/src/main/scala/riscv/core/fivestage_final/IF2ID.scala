...
import riscv.core.fivestage_final.InstructionsNop
...
class IF2ID extends Module {
  ...
  val instruction = Module(new PipelineRegister(
    width = Parameters.InstructionBits,
    defaultValue = InstructionsNop.nop
  ))
  ...
  val instruction_address = Module(new PipelineRegister(
    width = Parameters.AddrBits,
    defaultValue = ProgramCounter.EntryAddress
  ))
  ...
  val interrupt_flag = Module(new PipelineRegister(
    width = Parameters.InterruptFlagBits,
    defaultValue = 0.U(Parameters.InterruptFlagWidth)
  ))
