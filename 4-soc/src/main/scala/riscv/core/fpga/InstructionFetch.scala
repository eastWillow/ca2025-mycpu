...
class InstructionFetch extends Module {
  ...
  // IF1: request PC
  // -------------------------
  val if1_pc = RegInit(Parameters.EntryAddress)
  io.instruction_req := if1_pc
  ...
  val if2_btb_pred_taken  = RegInit(false.B)
  val if2_btb_pred_target = RegInit(0.U(Parameters.AddrWidth))

  // -------------------------
  // IF2: aligned PC (for rom_instruction)
  // -------------------------
  // BTB prediction: use predicted target if BTB predicts taken
  val if2_pc   = RegInit(Parameters.EntryAddress)
  val kill_if2 = RegInit(false.B)

  val advance = !io.stall_flag_ctrl && io.instruction_valid

  when(advance) {
    if2_pc              := if1_pc
    if2_btb_pred_taken  := btb.io.predicted_taken
    if2_btb_pred_target := btb.io.predicted_pc
  }

  io.instruction_address := if2_pc
  ...
  val speculative_ras_pop = is_return && if2_valid && !io.stall_flag_ctrl
  ...
  val is_indirect_jalr    = is_jalr && !is_return && if2_valid && !io.stall_flag_ctrl
  ...
  // Solving pipeline hazard
  val redirect_event =
    take_pending || take_btb_correction || take_current || ras_prediction_valid || ibtb_prediction_hit

  when(!io.stall_flag_ctrl) {
    val consume_kill = kill_if2 && io.instruction_valid
    kill_if2 := (kill_if2 && !consume_kill) || redirect_event
  }
