 ...
 * - Reduces branch penalty from 3 cycles to 2 cycle (single IF flush)
 * - Complex branches (BLT, BGE, BLTU, BGEU) still resolved in EX stage
 *
 * Jump Handling:
 * - JAL: Computed in ID stage, 2-cycle penalty
 ...
 * - Taken branch/jump: Flush IF stage only (2 bubble)
 * - Interrupt: Flush IF and ID stages (3 bubbles)
 ...
 * - Branch penalty: 2 cycle (simple branches), 3 cycles (complex branches)
 ...
 *
 * Interface (CPUBundle):
 * - instruction_req: PC to instruction memory
 * - instruction_address/instruction/instruction_valid: Instruction memory interface
...
class PipelinedCPU extends Module {
  ...
  val skidInst   = Reg(UInt(Parameters.InstructionWidth))
  val skidAddr   = Reg(UInt(Parameters.AddrWidth))
  val if2idStall = ctrl.io.if_stall || mem_stall

  // imem "return" side signals that feed IF2ID today
  val retInst = inst_fetch.io.id_instruction
  val retAddr = inst_fetch.io.instruction_address
  // capture exactly once when stall starts and we don't already have buffered inst
  when(if2idStall && !skidValid) {
    skidValid := true.B
    skidInst  := retInst
    skidAddr  := retAddr
  }

  // if IF2ID is flushing, buffered inst is no longer trustworthy
  when(if2id.io.flush) {
    skidValid := false.B
  }

  // drive IF2ID input: buffered one has priority
  val if2idInInst = Mux(skidValid, skidInst, retInst)
  val if2idInAddr = Mux(skidValid, skidAddr, retAddr)

  if2id.io.instruction         := if2idInInst
  if2id.io.instruction_address := if2idInAddr

  // once stall is released, let IF2ID consume the buffered inst for one cycle
  when(!if2idStall && skidValid && !if2id.io.flush) {
    skidValid := false.B
  }
