...
class IF2ID extends Module {
  ...
  // ============================================================
  // [CA25: Exercise 27] Valid-Bit IF/ID Flush
  // ============================================================
  // Hint: Do not synchronously reset the 32-bit instruction or predictor
  // payloads on flush. That fanout is the 100 MHz critical path (ID branch
  // compare → every IF/ID R/S pin).
  //
  // Rules:
  // 1. A 1-bit valid register captures 1, stalls with io.stall, flushes with
  //    io.flush. ID treats valid=0 as NOP.
  // 2. Payload registers (instruction, PC, predictor) stall only on io.stall.
  //    Their flush input stays false.
  //
  // Example: taken BEQ kills the next fetch by clearing valid, not by writing
  // 0x00000013 into the instruction register in the same cycle as the compare.
