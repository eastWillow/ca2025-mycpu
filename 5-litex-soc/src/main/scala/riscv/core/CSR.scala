...
class CSR extends Module {
  ...
  // ============================================================
  // [CA25: Exercise 30a] Sample CSR Performance-Counter Events
  // ============================================================
  // Hint: Pipeline events can originate on the fetch/redirect path. Sample
  // each enable with RegNext before it fans out into the 64-bit counter
  // increment network. Totals stay one-for-one; visibility is one cycle late.
  //
  // Connecting io.branch_misprediction combinationally to mhpmcounter3 was
  // Gate 1's first 100 MHz failure (WNS -2.604 ns into mhpmcounter6 enable).
  val branch_misprediction_event = ?
  val hazard_stall_event         = ?
  val memory_stall_event         = ?
  val control_stall_event        = ?
  val btb_miss_taken_event       = ?
  val branch_resolved_event      = ?
  val btb_predicted_event        = ?
