...
//  io.clint_jump_address := io.interrupt_handler_address
  ...
  // ============================================================
  // [CA25: Exercise 29] ID Jump Flag Without Trap OR
  // ============================================================
  // Hint: if_jump_flag is the ID-stage branch/JAL/JALR decision only.
  // InstructionFetch already has a dedicated trap PC mux from CLINT
  // (interrupt_assert / interrupt_handler_address). ORing interrupt_assert
  // here puts the handler mux on the compare → PC path of the IRQ variant.
  //
  // Do not: io.if_jump_flag := branch_taken || io.interrupt_assert
  // Do not mux if_jump_address to mtvec here.
  io.if_jump_flag := ?
