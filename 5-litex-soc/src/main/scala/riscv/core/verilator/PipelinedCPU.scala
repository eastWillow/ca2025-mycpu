...
class PipelinedCPU extends Module {
  ...
  // Read the physical register ports directly from the instruction fields.
  // InstructionDecode still masks unused operands for forwarding and hazard
  // detection, but putting that opcode-dependent mask in front of the register
  // file lengthens the ID branch-resolution feedback path into IF/ID flush.
  ...
  // ============================================================
  // [CA25: Exercise 30b] Two-Cycle IF/ID Valid Flush
  // ============================================================
  // Hint: Exercise 28 delays the PC redirect by one cycle. Hold the IF/ID
  // flush for that extra cycle so the sequential fetch issued before the
  // registered jump is discarded. Trap redirects still use need_if_flush.
  //
  // if2id.io.flush := current flush OR a one-cycle registered copy.
  val if_flush_hold = ?
  if2id.io.flush := ?
