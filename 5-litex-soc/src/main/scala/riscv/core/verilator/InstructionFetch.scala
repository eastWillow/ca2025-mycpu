...
class InstructionFetch extends Module {
  ...
  io.btb_predicted_taken  := false.B // btb.io.predicted_taken && io.instruction_valid
  ...
  io.ras_predicted_valid  := false.B // ras.io.valid && speculative_ras_pop && io.instruction_valid
  ...
  io.ibtb_predicted_valid  := false.B // ibtb_prediction_hit && io.instruction_valid
  ...
  // ============================================================
  // [CA25: Exercise 28] Registered PC Redirect
  // ============================================================
  // Hint: Do not feed the combinational ID compare (io.jump_flag_id /
  // io.jump_address_id) into pc.D. Use the already-registered
  // prev_jump_flag / prev_jump_addr. That cuts instruction → RF → compare
  // → PC on the 100 MHz IRQ image.
  //
  // Keep the trap mux combinatorial: io.interrupt_assert still redirects
  // in the MuxCase below. Skip take_current when BTB already predicted
  // correctly (registered).
  //
  // Out of scope: moving all branch resolution out of ID.
  val take_current =
    ? && !io.stall_flag_ctrl && !pending_jump && !RegNext(io.btb_correct_prediction, false.B)
  ...
  val default_next_pc = Mux(
    ...
    Mux(
      ...
      Mux(io.btb_predicted_taken, btb_next_pc, pc + 4.U) // BTB prediction or sequential
    )
  )
